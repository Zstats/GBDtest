...
VerifyManage <- R6::R6Class( "VerifyManage",
  public = list(
    ...
    encrypt_data = function(data) {
      # 使用默认的固定密钥和IV，而不是随机生成
      key <- self$DEFAULT_KEY
      iv <- self$DEFAULT_IV
      raw_data <- charToRaw(data)
      # cat("加密数据",openssl::base64_encode(data),"\n")
      # cat("加密密钥",openssl::base64_encode(key),"\n")
      # cat("加密IV",openssl::base64_encode(iv),"\n")

      # 调用动态库中的加密函数
      result <- base::.Call("r_encrypt_data", raw_data, key, iv, PACKAGE = "protectRule")
      base64Result <- openssl::base64_encode(result)
      base64Key <- openssl::base64_encode(key)
      base64Iv <- openssl::base64_encode(iv)
      # cat("加密结果",base64Result)
      return(list( data = base64Result, key = base64Key, iv = base64Iv))
    },
    ...
    decrypt_data = function(data) {
      ...
      # cat("解密数据", data, "\n")
      # cat("解密密钥", openssl::base64_encode(key), "\n")
      # cat("解密IV", openssl::base64_encode(iv), "\n")

      # 调用动态库中的解密函数
      result <- base::.Call("r_decrypt_data", raw_data, key, iv, PACKAGE = "protectRule")
